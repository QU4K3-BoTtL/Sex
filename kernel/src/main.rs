...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
    ...
    if sex_kernel::pku::is_pku_supported() {
        ...
        // Basic PKRU test
        let initial_pkru = sex_kernel::pku::Pkru::read();
        serial_println!("PKU: Initial PKRU value: {:#010x}", initial_pkru);
        
        // Disable write for key 1
        sex_kernel::pku::Pkru::set_permissions(1, false, true);
        let updated_pkru = sex_kernel::pku::Pkru::read();
        serial_println!("PKU: Updated PKRU value (Key 1 write disabled): {:#010x}", updated_pkru);
        
        // Restore permissions
        unsafe { sex_kernel::pku::Pkru::write(initial_pkru); }
